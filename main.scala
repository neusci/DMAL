...
class PN(id: Int) extends Actor {
  var time_cur:Double = 0.0 // temporal reference system (TRS) current time (ms)
  val data_writer = new PrintWriter(new File("pn"+id+".txt"))
  //  ...
  var V:Double = 0.0
  var w:Double = 0.0
  var V_next:Double = 0.0
  ...
  def dfdt_w() = {(w_inf(V)-w)/tau_w(V)}
  def I_Na() = {g_Na*pow(m_inf(V),3)*(1-w)*(V-E_Na)} // inward sodium current
  def I_K() = {g_K_PN*pow(w,4)*(V-E_K)} // next outward potassium current
  def I_L() = {g_L*(V-E_L)} // the next leak current
  def m_inf(v:Double):Double = {1/(1+ pow(al_m(v),2))}
  def w_inf(v:Double):Double = {1/(1+ pow(al_w(v),2))}
  def al_m(v:Double):Double = {exp((v-th_m)/k_m)}
  ...
  def iterate() = {
    ...
    V = V_next
    w = w_next
    printf("PN: %3d ; time (mS): %3.3f ; vol. (mV): %3.6f\n", id, time_cur, V)
  }
  // ...
  def receive() = {
    case Init() =>
      println("PN["+id+"] is ready.")
      printf("PN: %3d ; time (mS): %3.3f ; vol. (mV): %3.6f\n", id, time_cur, V)
      sender ! Done()
    case stim(i,x,t) =>
      I_app = x
  ...
  val PN_test = stagent.actorOf(Props( new PN(1) ))
...
*/
  ...
  PN_test ? Init() // run 10S:
  for (i <- 1 to 1000000) {
    PN_test ? stim(0, 0.2, 0)
    Thread sleep 1
  }
  PN_test ? Dest()
